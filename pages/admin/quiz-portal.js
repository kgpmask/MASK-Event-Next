...
export default function QuizPortalPage() {
	...
	useEffect(() => {
		const checkAdmin = async () => {
			try {
				const response = (await (await fetch("/api/check-admin")).json()).isAdmin;
				setIsAdmin(response);
	...
	const startQuiz = () => {
		const fetchQuestions = async () => {
			try {
				...
				if (!localStorage.getItem("questions") || !localStorage.getItem("questions").length) {
	...
	const startQuestion = async () => {
		try {
			...
			if (response1.status < 400) {
				...
				console.log(result2);
				setTimeout(
					() => { setDisabled(false); setQuestionState("Start Question")},
				...
				{start &&
					<div className={styles["question-info"]}>
						<div className={styles["round-info"]}>
							<p>{currentQ ? questions[currentQ]?.title.split(':')[0].trim() : questions[currentQ]?.title}</p>
							<h2>Shiri Masu Ka?</h2>
							<p>{`Question #${currentQ}`}</p>
						</div>
						{questionState === "Timer Started" && <Timer time={questions[currentQ]?.type === "mcq" ? 15 : 25}/>}
					</div>
				}
				<div className={styles["question"]}>
					...
					{start ?
						<div className={styles["quiz-nav-buttons"]}>
							<button className={currentQ ? '' : styles["disabled"]} onClick={() => currentQ ? setCurrentQ(currentQ - 1) : null}>Previous</button>
							<button
								disabled={disabled}
							...
							<button className={questions.length - (currentQ + 1) ? '' : styles["disabled"]} onClick={() => questions.length - (currentQ + 1) ? setCurrentQ(currentQ + 1) : null}>Next</button>
						</div>
						:
						<button onClick={startQuiz} className={styles["end-quiz"]}>Start Quiz</button>
					}
